...
class GapEncoderColumn(BaseEstimator, TransformerMixin):
    ...
    def _init_vars(self, X) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.hashing:
            ...
            self.ngrams_count_ = CountVectorizer(
                analyzer=self.analyzer, ngram_range=self.ngram_range, dtype=np.float32
            )
            if self.add_words:
                self.word_count_ = CountVectorizer(dtype=np.float32)
    ...
    def fit(self, X, y=None) -> "GapEncoderColumn":
        ...
        for n_iter_ in range(self.max_iter):
            ...
            W_change = np.linalg.norm(self.W_ - W_last) / np.linalg.norm(W_last)
                    # W_change = cp.linalg.norm(cp.subtract(self.W_, W_last)) / cp.linalg.norm(W_last)
    ...
    def get_feature_names_out(
        ...
        print('transform2')
        encoding = self.transform2(np.array(vocabulary).reshape(-1))
    ...
    def transform2(self, X) -> np.array:
        """
        Return the encoded vectors (activations) H of input strings in X.
        Given the learnt topics W, the activations H are tuned to fit V = HW.
        ...
        # for slc in gen_batches(n=unq_H.shape[0], batch_size=self.batch_size):
            # Given the learnt topics W, optimize H to fit V = HW
        unq_H = _multiplicative_update_h3(
            unq_V,#[slc],
            self.W_,
            unq_H,#[slc],
            epsilon=1e-3,
...
class vGapEncoder(BaseEstimator, TransformerMixin):
    ...
    The :class:`~cuCat.GapEncoder` supports online learning on batches of
    data for scalability through the :func:`~cuCat.GapEncoder.partial_fit`
    method.
    ...
    def fit(self, X, y=None) -> "vGapEncoder":
        ...
        :class:`~cuCat.GapEncoder`
            Fitted :class:`~cuCat.GapEncoder` instance (self).
        """
...
#     # Vt=csr_gpu(Vt)
#     A *= rho
#     A += cp.multiply(W, cp.matmul(Ht.T, (Vt * (1 / (cp.matmul(Ht, W) + 1e-10)))))
#     B *= rho
...
#     squared_epsilon = epsilon #**2 ## lessen threshold since we now take norm of full matrix
#     squared_norm = 1
#     Vt=csr_gpu(Vt);Ht=cp.array(Ht);W=cp.array(W);W_WT1=cp.array(W_WT1)
#     for n_iter_ in range(max_iter):
#         if squared_norm <= squared_epsilon:
#             break
#         R=cp.matmul(Ht, W)
#         ht_out = cp.multiply(Ht,cp.matmul(W_WT1, (Vt / ( R + 1e-10)).T).T) + const
#         squared_norm = cp.sum(cp.multiply(ht_out - Ht, ht_out - Ht) / cp.multiply(Ht, Ht))
#         Ht[:] = ht_out
#     ht=Ht.get()
#     del Ht,ht_out,W,Vt,W_WT1
#     cp._default_memory_pool.free_all_blocks()
#     return ht
...
def _multiplicative_update_w2(
    ...
    S=Ht.T.dot(R)
    # del R
    A += W.multiply(S) #+ 1e-10
    # del S
    B *= rho
    B += Ht.sum(axis=0).reshape(-1, 1)
    # print([B,B.data])
    # B=1/B
    W=A.multiply(1/B) #, out=W)
    if rescale_W:
        _rescale_W(W, A)
    # w=W.get(); a=A.get(); b=B.get()
    del W,A,B,Ht,Vt,QQ,R,S
    cp._default_memory_pool.free_all_blocks()
    return W,A,B #w,a,b

def _multiplicative_update_h2(
    Vt: np.array,
    W: np.array,
    Ht: np.array,
    epsilon: float = 1e-2,
    ...
    squared_epsilon = epsilon #**2 ## lessen threshold since we now take norm of full matrix
    squared_norm = 1
    # gVt=Vt.copy();gW=W.copy()#;gW_WT1=W_WT1.copy()
    Ht=csr_gpu(csr_cpu(Ht));Vt=csr_gpu(csr_cpu(Vt));W=csr_gpu(csr_cpu(W))
    # print([Vt.shape,Ht.shape,W.shape])#,W_WT1.shape])
    i=0
    for n_iter_ in range(max_iter):
        if squared_norm <= squared_epsilon:
            break
        # Vt=csr_gpu(csr_cpu(gVt));W=csr_gpu(csr_cpu(gW))
        # QQ=safe_sparse_dot(
        QQ=Ht.dot(W)#,dense_output=False)  # + 1e-10)
        QQ.data=1/QQ.data
        WW=W.multiply(1/(1 + 1/gamma_scale_prior))
        PP=Vt.multiply(QQ).transpose()
        # R=safe_sparse_dot(
        R=WW.dot(PP)#,dense_output=False)
        # del QQ,WW,PP
        
        # ht_out = Ht.multiply(csr_gpu(R.transpose())) # + const
        ht_out = Ht.multiply(csr_gpu(R.transpose())) # + const
        # del R
        
        squared_norm = ((ht_out - Ht).multiply(ht_out - Ht) / Ht.multiply(Ht)).sum()
        Ht[:] = ht_out
        # del ht_out
        cp._default_memory_pool.free_all_blocks()
        i=i+1
        print(i)
    # ht=Ht#.get()
    del ht_out,QQ,R,WW,PP,Vt,W
    cp._default_memory_pool.free_all_blocks()
    return Ht

def _multiplicative_update_h3(
    Vt: np.array,
    W: np.array,
    Ht: np.array,
    epsilon: float = 1e-3,
